...
import type { PaginationParams } from '$lib/shared/types/api.types'
...
function createTaskStore() {
	...
	async function createTask(payload: unknown): Promise<Task> {
		...
		try {
			...
			const newTask = await withErrorHandling(async () => {
				...
				state.tasks = state.tasks.map((task) => (task.id === tempId ? response : task))
	...
	async function updateTask(id: string, payload: unknown): Promise<Task> {
		...
		try {
			...
			const updatedTask = await withErrorHandling(async () => {
				// Make actual API call
				const response = await httpClient.put<Task>(`/tasks/${id}`, validatedPayload)

				state.tasks = state.tasks.map((task) => (task.id === id ? response : task))
				if (state.currentTask?.id === id) {
					state.currentTask = response
				}
	...
	async function deleteTask(id: string): Promise<void> {
		...
		try {
			...
			await withErrorHandling(async () => {
				// Make actual API call for deletion
	...
	async function restoreTask(id: string): Promise<void> {
		...
		try {
			await withErrorHandling(async () => {
				// Make actual API call
				await httpClient.put(`/tasks/${id}`, { status: 'todo' })
		...
		try {
			const subtask = await withErrorHandling(async () => {
				...
				if (state.currentTask?.id === taskId) {
					const updatedSubtasks = [...(state.currentTask.subtasks || []), response]
					const progress = calculateProgress(updatedSubtasks)
	...
	async function toggleSubtask(taskId: string, subtaskId: string): Promise<void> {
		...
		try {
			// Get current subtask state for API call
			const currentSubtask = state.tasks
				.find((t) => t.id === taskId)
				?.subtasks?.find((s) => s.id === subtaskId)
			const newIsCompleted = currentSubtask ? !currentSubtask.isCompleted : false
			...
			await withErrorHandling(async () => {
				// Make actual API call
				await httpClient.put(`/tasks/${taskId}/subtasks/${subtaskId}`, {
